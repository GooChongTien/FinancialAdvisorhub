...
async function seedKnowledgeAtoms() {
    ...
    // Step 0: Create Expert Brain Agent Config first
    console.log("Creating Expert Brain Agent Config...");
    const { data: agentConfig, error: agentError } = await supabase
        .from('mira_agent_configs')
        .insert({
            agent_id: 'expert_brain',
            module: 'knowledge',
            display_name: 'Expert Brain',
            system_prompt: 'You are the Expert Brain - a collection of mental models from experienced insurance advisors.',
            tools: [],
            parameters: {
                temperature: 0.7,
                model: 'gpt-4o-mini'
    ...
    if (agentError) {
        console.error("Error creating expert_brain agent config:", agentError);
        return;
    }
    ...
    if (wfError1) {
        ...
        console.log(`✅ Created workflow: ${wfProd01.name} (${wfProd01.id})`);
        ...
        for (const node of nodes) {
            ...
            else console.log(`  ✓ Created node: ${nodeRow.name}`);
        }
    ...
    console.log("\nCreating KA-FNA-01: Comprehensive Fact-Find...");
    ...
    if (wfError2) {
        ...
        console.log(`✅ Created workflow: ${wfFna01.name} (${wfFna01.id})`);
    ...
    console.log("\n🎯 Knowledge Atoms seeded successfully!");
    console.log("   Mira now has the 'mental models' of an expert advisor!");
}
